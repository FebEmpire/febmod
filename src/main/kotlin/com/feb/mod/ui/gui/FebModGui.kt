...
class FebModGui(initialTab: TopTab = TopTab.FEBMOD) : Screen(Component.literal("${ModInfo.MOD_NAME} ${ModInfo.VERSION}")) {
    ...
    private val logoTexture = Identifier.fromNamespaceAndPath("febmod", "textures/gui/feb_penguin.png") // I cba to add logo rn
    ...
    override fun extractRenderState(graphics: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        graphics.fill(0, 0, width, height, 0x991E3A5F.toInt())
        particles.update(width, height)
        particles.render(graphics)

        graphics.fill(0, TOP_BAR_HEIGHT, width, TOP_BAR_HEIGHT + 1, 0x60FFFFFF.toInt())
        graphics.fill(0, TOP_BAR_HEIGHT + 1, SIDEBAR_WIDTH, height, 0xCC15283F.toInt())
        graphics.fill(SIDEBAR_WIDTH, 0, SIDEBAR_WIDTH + 1, height, 0x60FFFFFF.toInt())

        if (selectedTopTab == TopTab.ADDONS) {
            graphics.fill(SIDEBAR_WIDTH + 1, TOP_BAR_HEIGHT + 1, SIDEBAR_WIDTH + ADDON_LIST_WIDTH, height, 0xBB172E4A.toInt())
            graphics.fill(SIDEBAR_WIDTH + ADDON_LIST_WIDTH, TOP_BAR_HEIGHT + 1, SIDEBAR_WIDTH + ADDON_LIST_WIDTH + 1, height, 0x60FFFFFF.toInt())
        }
